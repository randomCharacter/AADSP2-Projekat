...
void processing() {
	...
	for (i = 0; i < BLOCK_SIZE; i++) {
		if (output_mode != MODE2_0_0) {
			sampleBuffer[CH_C][i] = gain1 * second_order_IIR(sampleBuffer[CH_L][i], c_coefs, c_x_history, c_y_history);
			sampleBuffer[CH_LS][i] = gain1 * second_order_IIR(sampleBuffer[CH_L][i], ls_coefs, ls_x_history, ls_y_history);
			sampleBuffer[CH_RS][i] = gain2 * second_order_IIR(sampleBuffer[CH_L][i], rs_coefs, rs_x_history, rs_y_history);
		}
		if (output_mode == MODE3_2_1) {
			sampleBuffer[CH_LFE][i] = gain2 * second_order_IIR(sampleBuffer[CH_L][i], lfe_coefs, lfe_x_history, lfe_y_history);
		}
		sampleBuffer[CH_R][i] = gain2 * second_order_IIR(sampleBuffer[CH_L][i], r_coefs, r_x_history, r_y_history);
		sampleBuffer[CH_L][i] = gain1 * second_order_IIR(sampleBuffer[CH_L][i], l_coefs, l_x_history, l_y_history);
	}
}
